...
async function captureTurn(client, threadId, startRequest, options = {}) {
  ...
  try {
    ...
    const transportExit = client.exitPromise.then(() => {
      if (state.completed) {
        return state;
      }
      throw (
        client.exitError ??
        new Error("Codex app-server transport closed before the turn completed.")
      );
    });
    return await Promise.race([state.completion, transportExit]);
  } finally {
    clearCompletionTimer(state);
...
async function withAppServer(cwd, fn, options = {}) {
  ...
  try {
    client = await CodexAppServerClient.connect(cwd, {
      disableBroker: Boolean(options.disableBroker),
    });
