...
function cleanupSessionJobs(cwd, sessionId) {
  ...
  const preservedJobIds = new Set(
    sessionJobs
      .filter(
        (job) => (job.status === "queued" || job.status === "running") && isBackgroundJob(job),
      )
      .map((job) => job.id),
  );
  const removedJobs = sessionJobs.filter((job) => !preservedJobIds.has(job.id));
  ...
  saveState(workspaceRoot, {
    ...state,
    jobs: state.jobs.filter((job) => job.sessionId !== sessionId || preservedJobIds.has(job.id)),
  });
}
