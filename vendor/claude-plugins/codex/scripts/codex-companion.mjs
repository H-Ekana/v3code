...
import { LOG_FILE_ENV, PID_FILE_ENV } from "./lib/broker-lifecycle.mjs";
import {
...
function getTaskWorkerCommandIncludes(jobId) {
  return [path.join(ROOT_DIR, "scripts", "codex-companion.mjs"), "task-worker", "--job-id", jobId];
}
...
function spawnDetachedTaskWorker(cwd, jobId) {
  ...
  delete env[PID_FILE_ENV];
  delete env[LOG_FILE_ENV];
  const child = spawn(
    process.execPath,
  ...
  const processIdentity = {
    pid: child.pid ?? null,
    commandIncludes: getTaskWorkerCommandIncludes(jobId),
  };
  child.unref();
...
function enqueueBackgroundTask(cwd, job, request) {
  ...
  const { child, processIdentity } = spawnDetachedTaskWorker(cwd, job.id);
  const queuedRecord = {
    ...job,
    background: true,
    status: "queued",
...
async function handleTaskWorker(argv) {
  ...
  const cwd = resolveCommandCwd(options);
  const workspaceRoot = resolveCommandWorkspace(options);
  let storedJob = readStoredJob(workspaceRoot, options["job-id"]);
  for (let attempt = 0; !storedJob && attempt < 40; attempt += 1) {
    await new Promise((resolve) => setTimeout(resolve, 50));
    storedJob = readStoredJob(workspaceRoot, options["job-id"]);
  }
  if (!storedJob) {
  ...
  const background = isBackgroundJob(storedJob);
  const processIdentity = captureProcessIdentity(process.pid, {
    commandIncludes: getTaskWorkerCommandIncludes(storedJob.id),
  });
...
function buildExpectedJobProcess(job) {
  if (job.processIdentity) {
    return job.processIdentity;
  }
  if (isBackgroundJob(job)) {
    return { commandIncludes: getTaskWorkerCommandIncludes(job.id) };
  }
  return {
    commandIncludes: [
      path.join(ROOT_DIR, "scripts", "codex-companion.mjs"),
...
async function handleCancel(argv) {
  ...
  const expectedProcess = buildExpectedJobProcess({ ...job, ...existing });
  const verification = verifyProcessIdentity(job.pid ?? Number.NaN, expectedProcess);
  if (!verification.matches) {
    appendLogLine(job.logFile, `Cancellation refused: ${verification.detail}`);
    throw new Error(`Refusing to cancel ${job.id}: ${verification.detail}`);
  }
  ...
  const termination = terminateProcessTree(job.pid ?? Number.NaN, { expectedProcess });
  if (termination.refused) {
    appendLogLine(job.logFile, `Cancellation refused: ${termination.detail}`);
    throw new Error(`Refusing to cancel ${job.id}: ${termination.detail}`);
  }
