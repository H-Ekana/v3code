...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  ...
  // Fork-specific: our branding, and the crash-loop ceiling that stops a broken
  // install from restarting every 5s and growing the unrotated append log.
  expect(unit).toContain("Description=V3 Code server (T3 Connect)");
