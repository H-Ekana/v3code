...
const make = Effect.gen(function* () {
  ...
  const start: ProviderCommandReactorShape["start"] = Effect.fn("start")(function* () {
    ...
    // Parked rather than started immediately, matching every other root in this
    // file. `Stream.fromPubSub` still subscribes lazily, so the ordering our
    // `startImmediately: true` used to enforce (b255e32bd) now rests on
    // upstream's activation boundary: commands are only accepted after
    // activation has released these fibers.
