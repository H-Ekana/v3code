  ...
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        ...
        "/repo/assets/v3/v3-code-nightly-v2-1024.png",
      );
      assert.equal(environment.backendEntryPath, "/repo/apps/server/dist/bin.mjs");
      assert.equal(environment.backendCwd, "/repo");
      assert.equal(environment.appUserModelId, "com.v3code.desktop.dev");
      assert.equal(environment.linuxWmClass, "v3code-dev");
      assert.deepEqual(
