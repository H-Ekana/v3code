                ...
                {props.settlementSupported || showSnoozeButton ? (
                  <span
                    className={cn(
                      ...
                      props.settlePhase !== "idle" && "static opacity-100",
                    )}
                  >
                    {showSnoozeButton ? (
                      <SnoozePopoverButton
  ...
  const { pinnedThreads, activeThreads, snoozedThreads, settledThreads, snoozeNow } =
    useMemo(() => {
      ...
      for (const thread of visible) {
        ...
        // Precedence, strongest first: held > snoozed > pinned > settled >
        // active.
        //
        // A just-acknowledged single settle is HELD in its active slot while
        // its ring laps in place. The store already classifies the thread as
        // settled, but the row must not regroup — and so must not FLIP down
        // into the shelf — until the hold window ends (see
        // THREAD_SETTLE_HOLD_MS and the release timer below). Held outranks
        // pin, snooze and settled classification alike.
        if (heldSettleKeys.has(threadKey)) {
          active.push(thread);
          // Snooze outranks everything below it, including a pin: "hide until
          // Tuesday" temporarily suspends "keep on top". The pin survives
          // underneath — pinned cards are creation-ordered, so on wake the
          // thread reappears at its original spot in the pinned block. (For
          // unpinned threads this is also the snooze-beats-auto-settle rule:
          // the wake time is a stronger statement about when the thread
          // matters again.)
      ...
      return {
        ...
        // it does not introduce a new ordering scheme — so no manual order
        // here, unlike the inbox below.
                  ...
                  >
                    {(() => {
                      const renderThreadRow = (
                        ...
                        // Only inbox cards are rearrangeable. The snoozed and
                        // settled shelves are time-ordered history, where a
                        // hand-placed row would mean nothing; the pinned block
                        // is creation-ordered by definition, so its cards stay
                        // out of the active list's SortableContext.
