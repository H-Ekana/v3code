...
import { getKeyboardResizedWidth } from "~/hooks/useResizableWidth";
...
const SIDEBAR_RESIZE_DEFAULT_MAX_WIDTH = 64 * 16;
const SIDEBAR_RESIZE_KEYBOARD_STEP = 8;
const SIDEBAR_RESIZE_KEYBOARD_LARGE_STEP = 32;
  ...
  return (
    <SidebarInstanceContext value={instanceContextValue}>
      ...
      >
        ...
        <div
          className={cn(
            "relative w-(--sidebar-width) bg-transparent transition-[width] [transition-duration:220ms] [transition-timing-function:var(--ease-out-quart,cubic-bezier(0.25,1,0.5,1))] motion-reduce:transition-none",
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] [transition-duration:220ms] [transition-timing-function:var(--ease-out-quart,cubic-bezier(0.25,1,0.5,1))] motion-reduce:transition-none md:flex",
            side === "left"
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      className={cn(
        "size-[var(--workspace-titlebar-control-size)]! overflow-hidden [-webkit-app-region:no-drag]",
        className,
      )}
    ...
    >
      <span className="relative size-4" aria-hidden>
        <PanelLeftCloseIcon className="sidebar-trigger-icon sidebar-trigger-icon-open absolute inset-0 size-4" />
        <PanelLeftIcon className="sidebar-trigger-icon sidebar-trigger-icon-closed absolute inset-0 size-4" />
      </span>
...
function SidebarRail({
  ...
  const transitionResetFrameRef = React.useRef<number | null>(null);
  const [currentWidth, setCurrentWidth] = React.useState(SIDEBAR_RESIZE_DEFAULT_MIN_WIDTH);
  ...
  const stopResize = React.useCallback(
    (pointerId: number) => {
      ...
      setCurrentWidth(resizeState.width);
      setIsResizing(false);
  ...
  const handleKeyDown = React.useCallback(
    (event: React.KeyboardEvent<HTMLButtonElement>) => {
      ...
      const measuredWidth = clampSidebarWidth(
        sidebarContainer.getBoundingClientRect().width || currentWidth,
        resolvedResizable,
      );
      const nextWidth = getKeyboardResizedWidth({
        ...
        maxWidth: resolvedResizable.maxWidth,
        step: SIDEBAR_RESIZE_KEYBOARD_STEP,
        largeStep: SIDEBAR_RESIZE_KEYBOARD_LARGE_STEP,
    ...
    [currentWidth, onKeyDown, open, resolvedResizable, sidebarInstance?.side],
  );
        ...
        render={
          <button
            ...
            className={cn(
              ...
              "sidebar-resize-rail -translate-x-1/2 group-data-[side=left]:-right-4 absolute inset-y-0 z-20 hidden w-4 outline-none transition-colors motion-reduce:transition-none group-data-[side=right]:left-0 sm:flex [[data-collapsible=offcanvas][data-state=collapsed]_&]:pointer-events-none",
            ...
            aria-valuemax={canResize ? resolvedResizable.maxWidth : undefined}
            aria-valuenow={
              canResize ? Math.round(clampSidebarWidth(currentWidth, resolvedResizable)) : undefined
            }
            onClick={handleClick}
            ...
            type="button"
          >
            <span className="sidebar-resize-tip" aria-hidden />
          </button>
        }
      />
...
function SidebarGroupAction({ className, render, ...props }: useRender.ComponentProps<"button">) {
  const defaultProps = {
    className: cn(
      "absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-lg p-0 text-sidebar-foreground outline-hidden ring-ring transition-[transform,background-color,color,box-shadow] duration-150 hover:bg-sidebar-row-hover hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-primary/50 motion-reduce:transition-none [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
...
function SidebarMenuAction({
  ...
  const defaultProps = {
    className: cn(
      "absolute top-1.5 right-1 flex aspect-square w-5 cursor-pointer items-center justify-center rounded-lg p-0 text-sidebar-foreground outline-hidden ring-ring transition-[transform,background-color,color,box-shadow] duration-150 hover:bg-sidebar-row-hover hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-primary/50 peer-hover/menu-button:text-sidebar-foreground motion-reduce:transition-none [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
      // Increases the hit area of the button on mobile.
      "after:-inset-2 after:absolute md:after:hidden",
...
function SidebarMenuSubButton({
  ...
  const defaultProps = {
    className: cn(
      "-translate-x-px flex h-7 min-w-0 cursor-pointer items-center gap-2 overflow-hidden rounded-lg px-2 text-sidebar-foreground outline-hidden ring-ring transition-[background-color,color,box-shadow] duration-150 hover:bg-sidebar-row-hover hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-primary/50 active:bg-sidebar-row-active active:text-sidebar-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 motion-reduce:transition-none [&>span:last-child]:truncate [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-muted-foreground",
      "data-[active=true]:bg-primary/10 data-[active=true]:text-sidebar-foreground data-[active=true]:ring-1 data-[active=true]:ring-primary/15 dark:data-[active=true]:bg-primary/15",
      size === "sm" && "text-xs",
      size === "md" && "text-sm",
      "group-data-[collapsible=icon]:hidden",
      className,
