...
const buttonVariants = cva(
  // Upstream's control-geometry tokens (`--control-radius`, `--control-icon-color`)
  // with our motion utilities in place of Tailwind's focus ring: `.motion-focus`
  // already draws the focus ring and transitions box-shadow, so keeping
  // `focus-visible:ring-*` and `transition-shadow` would double the ring and
  // narrow the transition to box-shadow alone. Icon dimming is upstream's now —
  // `opacity-80` gave way to the semantic color variable on purpose.
  "motion-focus motion-hover motion-press [--control-icon-color:currentColor] [&_svg]:-mx-0.5 relative inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-[var(--control-radius)] border font-medium text-base outline-none before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--control-radius)-1px)] pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 disabled:pointer-events-none disabled:opacity-64 sm:text-sm [&_svg:not([class*='text-'])]:text-[var(--control-icon-color)] [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    defaultVariants: {
      size: "default",
      variant: "default",
