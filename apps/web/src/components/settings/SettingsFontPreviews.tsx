...
// The preview is a font sample, not a real draft: large-paste collapsing is
// off, so pasting a wall of text here shows the glyphs at size rather than
// swallowing them into a chip.
const EMPTY_LARGE_PASTES: ReadonlyArray<never> = [];
  ...
  return (
    <div className="mt-1 mb-2 rounded-lg border border-border bg-background px-3 py-2">
      <ComposerPromptEditor
        ...
        largePasteEnabled={false}
        skills={EMPTY_SKILLS}
        disabled={false}
        placeholder="Ask for follow-up changes or attach images"
        className="max-h-40 min-h-12"
        onRemoveTerminalContext={noop}
        onAddLargePaste={noop}
        onRemoveLargePaste={noop}
        onChange={onChange}
        onPaste={noop}
      />
    </div>
