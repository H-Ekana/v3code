...
const WORKTREE_NAME = "subagent-sidebar-preview";
...
export function resolveV3WorkspaceRoot(invocationRoot) {
  ...
  const worktreeRoot = join(currentRoot, ".worktrees", WORKTREE_NAME);
  if (isV3Workspace(worktreeRoot)) return worktreeRoot;

  throw new Error(
    `Could not find the V3 Code worktree. Expected ${worktreeRoot} or a V3 Code checkout as the current directory.`,
  );
}
